...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLed_WarmSta)
    {
        info->event[info->num] = EVENT_Led_WarmSta;
        info->num++;
        dataPoints->valueLed_WarmSta = gizStandardDecompressionValue(Led_WarmSta_BYTEOFFSET,Led_WarmSta_BITOFFSET,Led_WarmSta_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagLed_RVal)
    {
        info->event[info->num] = EVENT_Led_RVal;
        info->num++;
        dataPoints->valueLed_RVal = gizX2Y(Led_RVal_RATIO,  Led_RVal_ADDITION, issuedData->attrVals.valueLed_RVal); 
    }
        
    if(0x01 == issuedData->attrFlags.flagLed_GVal)
    {
        info->event[info->num] = EVENT_Led_GVal;
        info->num++;
        dataPoints->valueLed_GVal = gizX2Y(Led_GVal_RATIO,  Led_GVal_ADDITION, issuedData->attrVals.valueLed_GVal); 
    }
        
    if(0x01 == issuedData->attrFlags.flagLed_BVal)
    {
        info->event[info->num] = EVENT_Led_BVal;
        info->num++;
        dataPoints->valueLed_BVal = gizX2Y(Led_BVal_RATIO,  Led_BVal_ADDITION, issuedData->attrVals.valueLed_BVal); 
    }
        
    if(0x01 == issuedData->attrFlags.flagLed_Brightness)
    {
        info->event[info->num] = EVENT_Led_Brightness;
        info->num++;
        dataPoints->valueLed_Brightness = gizX2Y(Led_Brightness_RATIO,  Led_Brightness_ADDITION, exchangeBytes(issuedData->attrVals.valueLed_Brightness));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLed_WarmSta != cur->valueLed_WarmSta)
    {
        GIZWITS_LOG("valueLed_WarmSta Changed\n");
        ret = 1;
    }
    ...
    if(last->valueLed_RVal != cur->valueLed_RVal)
    {
        GIZWITS_LOG("valueLed_RVal Changed\n");
        ret = 1;
    }
    if(last->valueLed_GVal != cur->valueLed_GVal)
    {
        GIZWITS_LOG("valueLed_GVal Changed\n");
        ret = 1;
    }
    if(last->valueLed_BVal != cur->valueLed_BVal)
    {
        GIZWITS_LOG("valueLed_BVal Changed\n");
        ret = 1;
    }
    if(last->valueLed_Brightness != cur->valueLed_Brightness)
    {
        GIZWITS_LOG("valueLed_Brightness Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Led_WarmSta_BYTEOFFSET,Led_WarmSta_BITOFFSET,Led_WarmSta_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLed_WarmSta);
    ...
    devStatusPtr->valueLed_RVal = gizY2X(Led_RVal_RATIO,  Led_RVal_ADDITION, dataPoints->valueLed_RVal); 
    devStatusPtr->valueLed_GVal = gizY2X(Led_GVal_RATIO,  Led_GVal_ADDITION, dataPoints->valueLed_GVal); 
    devStatusPtr->valueLed_BVal = gizY2X(Led_BVal_RATIO,  Led_BVal_ADDITION, dataPoints->valueLed_BVal); 

    devStatusPtr->valueLed_Brightness = exchangeBytes(gizY2X(Led_Brightness_RATIO,  Led_Brightness_ADDITION, dataPoints->valueLed_Brightness)); 
