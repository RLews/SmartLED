...
static const char *gGizLogVersion = "2.1.1.16120823"; //GizWits日志版本号

typedef struct {
    char *buf; //__android_log_print最多只能输出LOG_MAX_LEN(=1024)字节的日志
    int bufSize;
    void *next;
    int printLevelRequired;
    int androidLevel;
    char mode;
} GizSDKLogSysList;

static pthread_t _sysLogThread = 0;
static GizSDKLogSysList *_listBegin = NULL; //先入先出的链表
static GizSDKLogSysList *_listEnd = NULL;
static pthread_mutex_t _sysLogThreadHandler = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t _listHandler = PTHREAD_MUTEX_INITIALIZER;
    
#define GIZ_LOG_MODE_SYS 0
#define GIZ_LOG_MODE_BIZ 1
    
static void logCheckSys(void);
    
static void sysAddListItem(GizSDKLogSysList *item) {
    pthread_mutex_lock(&_listHandler);
    if (!_listBegin) {
        _listBegin = item;
        _listEnd = item;
    } else if (_listEnd) { //始终保证在最后插入
        _listEnd->next = item;
        _listEnd = item;
    }
    pthread_mutex_unlock(&_listHandler);
}
    
static GizSDKLogSysList *sysRemoveFirstItem() {
    GizSDKLogSysList *ret = NULL;
    pthread_mutex_lock(&_listHandler);
    ret = _listBegin;
    if (_listBegin) {
        _listBegin = _listBegin->next;
    }
    pthread_mutex_unlock(&_listHandler);
    return ret;
}
    
static void *sysLogThread(void *p) {
    pthread_detach(pthread_self());
        
    GizSDKLogSysList *item = NULL;
    do {
        item = sysRemoveFirstItem();
        if (!item) {
            usleep(100000);
            continue;
        }
        //屏幕输出
        if (gGizLog.printLevel >= item->printLevelRequired) {
#ifdef __ANDROID__
            __android_log_print(item->androidLevel, LOG_TAG, "%s", item->buf);
#else
            fprintf(stdout, "%s\n", item->buf);
#endif
        }
...
static void printLog(GizSDKLogSysList *sysList, char mode, int printLevelRequired) {
    if (!sysList->buf) {
        free(sysList);
        return;
    }

    sysList->printLevelRequired = printLevelRequired;
    sysList->mode = mode;
    sysList->next = NULL;
    sysAddListItem(sysList);
        
    pthread_mutex_lock(&_sysLogThreadHandler);
    if (!_sysLogThread) {
        if (pthread_create(&_sysLogThread, NULL, sysLogThread, NULL)) {
            free(sysList->buf);
            free(sysList);
        }
    }
    pthread_mutex_unlock(&_sysLogThreadHandler);
}

static GizSDKLogSysList *GizSDKLogSysListInit() {
    GizSDKLogSysList *sysList = malloc(sizeof(GizSDKLogSysList));
    if (sysList) {
        sysList->bufSize = 1024; //先分配1K，满足大多数的需要
        sysList->buf = malloc(sysList->bufSize);
        if (!sysList->buf) {
            free(sysList);
            sysList = NULL;
        }
...
static void processNewLine(GizSDKLogSysList *list)
{
    char *pNewLine = NULL;
    pNewLine = strchr(list->buf, '\n');
    if (pNewLine) {
        int offset = (int)(pNewLine - list->buf);
        char *tmpBuf = malloc(list->bufSize);
        if (!tmpBuf) {
            return;
        }
        list->bufSize += 2;
        list->buf = realloc(list->buf, list->bufSize);
        if (list->buf) {
            pNewLine = list->buf + offset;
            strcpy(tmpBuf, pNewLine + 1);
            pNewLine[0] = ']';
            pNewLine[1] = '\n';
            strcpy(pNewLine + 2, tmpBuf);
            list->buf[strlen(list->buf) - 1] = '\0';
        }
        free(tmpBuf);
    }
}
    
void GizPrint(GIZ_LOG_TYPE logType, const char *format, ...) {
    GizSDKLogSysList *sysList = GizSDKLogSysListInit();
    va_list args;
    if (!sysList) {
        return;
    }
    ...
    //日志格式化
    do { //第一次的缓冲区不够，按照实际的缓冲区大小分配，有一个分配上限LOG_MAX_LEN
        va_start(args, format);
        int len = vsnprintf(sysList->buf, sysList->bufSize, format, args);
        if (len < 0) {
            free(sysList->buf);
            free(sysList);
            va_end(args);
        ...
        if (sysList->bufSize != LOG_MAX_LEN && len+1 > sysList->bufSize) {
            sysList->bufSize = len+1;
            if (sysList->bufSize > LOG_MAX_LEN) {
                sysList->bufSize = LOG_MAX_LEN;
            }
            sysList->buf = realloc(sysList->buf, sysList->bufSize);
            if (!sysList->buf) {
                free(sysList);
                va_end(args);
                return;
    ...
    if (sysList->buf[sysList->bufSize - 2]) sysList->buf[sysList->bufSize - 2] = ']'; //缓存区占满情况下追加结束符
    //遇到换行符的情况下将换行符后的内容当做详细日志单独打印(不用[]包)
    processNewLine(sysList);

    switch (logType) {
        case GIZ_LOG_TYPE_DEBUG:
#ifdef __ANDROID__
            sysList->androidLevel = ANDROID_LOG_DEBUG;
#endif
            printLog(sysList, GIZ_LOG_MODE_SYS, 2);
            break;
        case GIZ_LOG_TYPE_ERROR:
#ifdef __ANDROID__
            sysList->androidLevel = ANDROID_LOG_ERROR;
#endif
            printLog(sysList, GIZ_LOG_MODE_SYS, 1);
            break;
        case GIZ_LOG_TYPE_BIZ:
#ifdef __ANDROID__
            sysList->androidLevel = ANDROID_LOG_INFO;
#endif
            printLog(sysList, GIZ_LOG_MODE_BIZ, 1);
            break;
        default:
            free(sysList->buf);
            free(sysList);
...
void GizPrintFromUp(GIZ_LOG_TYPE type, const char *content) {
    GizSDKLogSysList *sysList = GizSDKLogSysListInit();
    if (!sysList) {
        return;
    }
    ...
    do { //第一次的缓冲区不够，按照实际的缓冲区大小分配，有一个分配上限LOG_MAX_LEN
        int len = (int)strncpy(sysList->buf, content, sysList->bufSize);
        if (len < 0) {
            free(sysList->buf);
            free(sysList);
            return;
    ...
    if (sysList->buf[sysList->bufSize - 1]) sysList->buf[sysList->bufSize - 1] = ']'; //缓存区占满情况下追加结束符
    //遇到换行符的情况下将换行符后的内容当做详细日志单独打印(不用[]包)
    processNewLine(sysList);
...
{
    GizPrintFromUp(GIZ_LOG_TYPE_BIZ, content);
}
...
{
    GizPrintFromUp(GIZ_LOG_TYPE_ERROR, content);
}
...
{
    GizPrintFromUp(GIZ_LOG_TYPE_DEBUG, content);
}
