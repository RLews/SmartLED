...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 * ===========================================================
	 */
	// 数据点"Led_WarmSta"对应的标识名
	protected static final String KEY_LED_WARMSTA = "Led_WarmSta";
	// 数据点"LedOnOff"对应的标识名
	protected static final String KEY_LEDONOFF = "LedOnOff";
	// 数据点"Led_RVal"对应的标识名
	protected static final String KEY_LED_RVAL = "Led_RVal";
	// 数据点"Led_GVal"对应的标识名
	protected static final String KEY_LED_GVAL = "Led_GVal";
	// 数据点"Led_BVal"对应的标识名
	protected static final String KEY_LED_BVAL = "Led_BVal";
	// 数据点"Led_Brightness"对应的标识名
	protected static final String KEY_LED_BRIGHTNESS = "Led_Brightness";
	 ...
	 * ===========================================================
	 */
	// 数据点"Led_RVal"对应seekbar滚动条补偿值
	protected static final int LED_RVAL_OFFSET = 0;
	// 数据点"Led_RVal"对应数据点增量值
	protected static final int LED_RVAL_ADDITION = 0;
	// 数据点"Led_RVal"对应数据点定义的分辨率
	protected static final int LED_RVAL_RATIO = 1;
		
	// 数据点"Led_GVal"对应seekbar滚动条补偿值
	protected static final int LED_GVAL_OFFSET = 0;
	// 数据点"Led_GVal"对应数据点增量值
	protected static final int LED_GVAL_ADDITION = 0;
	// 数据点"Led_GVal"对应数据点定义的分辨率
	protected static final int LED_GVAL_RATIO = 1;
		
	// 数据点"Led_BVal"对应seekbar滚动条补偿值
	protected static final int LED_BVAL_OFFSET = 0;
	// 数据点"Led_BVal"对应数据点增量值
	protected static final int LED_BVAL_ADDITION = 0;
	// 数据点"Led_BVal"对应数据点定义的分辨率
	protected static final int LED_BVAL_RATIO = 1;
		
	// 数据点"Led_Brightness"对应seekbar滚动条补偿值
	protected static final int LED_BRIGHTNESS_OFFSET = 0;
	// 数据点"Led_Brightness"对应数据点增量值
	protected static final int LED_BRIGHTNESS_ADDITION = 0;
	// 数据点"Led_Brightness"对应数据点定义的分辨率
	protected static final int LED_BRIGHTNESS_RATIO = 1;
	 ...
	 * ===========================================================
	 */
	// 数据点"Led_WarmSta"对应的存储数据
	protected static boolean data_Led_WarmSta;
	// 数据点"LedOnOff"对应的存储数据
	protected static boolean data_LedOnOff;
	// 数据点"Led_RVal"对应的存储数据
	protected static int data_Led_RVal;
	// 数据点"Led_GVal"对应的存储数据
	protected static int data_Led_GVal;
	// 数据点"Led_BVal"对应的存储数据
	protected static int data_Led_BVal;
	// 数据点"Led_Brightness"对应的存储数据
	protected static int data_Led_Brightness;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_LED_WARMSTA)) {
					data_Led_WarmSta = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_LEDONOFF)) {
					data_LedOnOff = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_LED_RVAL)) {
			
					data_Led_RVal = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_LED_GVAL)) {
			
					data_Led_GVal = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_LED_BVAL)) {
			
					data_Led_BVal = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_LED_BRIGHTNESS)) {
			
					data_Led_Brightness = (Integer) map.get(dataKey);
				}
			}
		}
