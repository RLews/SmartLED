...
void Drv_AdcInit(void)
{
	...
	for (i = 0; i < (uint8_t)EN_ADC_ALL_SIGNAL; i++)
	{
		...
		ADC_InitStructure.ADC_ScanConvMode = (FunctionalState)pAdcConfig[i].adcScanConvMode;
		ADC_InitStructure.ADC_ContinuousConvMode = (FunctionalState)pAdcConfig[i].adcContConvMode;
