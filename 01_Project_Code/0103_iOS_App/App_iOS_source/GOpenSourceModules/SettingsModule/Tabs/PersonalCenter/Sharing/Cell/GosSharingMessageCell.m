...
- (void)layoutSubviews {
    ...
    if (self.isAcceptRequired) { //小圆点
        [GosCommon markTableViewCell:self label:self.labelContent hasUnreadMessage:YES];
    } else {
        [GosCommon markTableViewCell:self label:self.labelContent hasUnreadMessage:NO];
    }
}
    ...
    if (sharingInfo.status == GizDeviceSharingNotAccepted && timerInterval > 0) {
        ...
        switch (sharingInfo.status) {
            ...
            case GizDeviceSharingNotAccepted:
                if (timerInterval < 0) {
                    status = NSLocalizedString(@"Sharing Timeout", nil);
                }
                break;
        }
...
- (IBAction)onAccept:(id)sender { //接口事件在控制器中接收
    [GosCommon showHUDAddedTo:self.listView animated:YES];
    [GizDeviceSharing acceptDeviceSharing:common.token sharingID:self.sharingInfo.id accept:YES];
}

- (IBAction)onDecline:(id)sender { //接口事件在控制器中接收
    [GosCommon showHUDAddedTo:self.listView animated:YES];
    [GizDeviceSharing acceptDeviceSharing:common.token sharingID:self.sharingInfo.id accept:NO];
}
