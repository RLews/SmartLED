...
* Output Arguments : 
* Returns          : 
* Notes            : wifi work mode only judge led flash completed.
* Author           : Lews Hammond
* Time             : 2019-6-27
************************************************************************************************************************
*/
...
* Description      : wifi led main switch handle
* Input Arguments  : 
* Output Arguments : 
* Returns          : 
* Notes            : 
	...
	if (Hal_CheckNewKey(EN_KEY_WIFI) == EN_STD_TRUE)
	{
		switch (Hal_GetKeySta(EN_KEY_WIFI))
		{
			case EN_KEY_PRESS_UP:
				...
				printf("\nKey Press.\n");
#endif
				/* enable or disable shock scan and led  */
				if (pSet->setMode == EN_WIFI_MODE_RUN)
				{
					pSet->setMode = EN_WIFI_MODE_SLEEP;
					Led_AllOff();
					Shk_DisableScan();
				}
				else if (pSet->setMode == EN_WIFI_MODE_SLEEP)
				{
					pSet->setMode = EN_WIFI_MODE_RUN;
					Led_ModeCycling();
					Shk_EnableScan();
