...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLedStatusFlag)
    {
        info->event[info->num] = EVENT_LedStatusFlag;
        info->num++;
        dataPoints->valueLedStatusFlag = gizStandardDecompressionValue(LedStatusFlag_BYTEOFFSET,LedStatusFlag_BITOFFSET,LedStatusFlag_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLedStatusFlag != cur->valueLedStatusFlag)
    {
        GIZWITS_LOG("valueLedStatusFlag Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(LedStatusFlag_BYTEOFFSET,LedStatusFlag_BITOFFSET,LedStatusFlag_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLedStatusFlag);
